...
import numpy as np
import random
from mymodel_deb import AGRestormer
...
parser.add_argument('-data_dir', help='Directory for validation/test data', default='./data/testDS/', type=str)
...
data_loader = DataLoader(
    myDataloader(data_dir, mode="val", data_types=["raindrop"]),
...
with torch.no_grad():  # Disable gradient computation
    for idx, (input_image, gt_image) in enumerate(data_loader):
        ...
        for i in range(input_image.size(0)):  # Handle batch size > 1
            input_save_path = os.path.join(args.output_dir, f"{idx+i}_input.png")
            output_save_path = os.path.join(args.output_dir, f"{idx+i}_output.png")
            gt_save_path = os.path.join(args.output_dir, f"{idx+i}_gt.png")
            visualization_path = os.path.join(args.output_dir, f"{idx+i}_visualization.png")
            ...
            gt_image_0 = inverse_transform(gt_image[i].cpu())
