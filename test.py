...
parser.add_argument('-data_dir', help='Directory for validation/test data', default='./data/testDS/',required= True, type=str)
parser.add_argument('-output_dir', help='Directory for your output data', default='./data/output',required= True, type=str)
parser.add_argument('-gt_dir', help='Directory for gt of your input data', default=None, type=str)
...
data_loader = DataLoader(
    myDataloader(data_dir, mode="test", test_gt_path=gt_dir),
    batch_size=batch_size,
    shuffle=False,
    num_workers=0
)
file_path = [f for f in os.listdir(data_dir) if f.endswith('.png')]
file_name = [os.path.splitext(f)[0] for f in file_path]
...
with torch.no_grad():  # Disable gradient computation
    for idx, (input_image, gt_image) in enumerate(data_loader):
        ...
        for i in range(input_image.size(0)):  # Handle batch size > 1
            input_save_path = os.path.join(args.output_dir, f"{file_name[idx+i]}_input.png")
            output_save_path = os.path.join(args.output_dir, f"{file_name[idx+i]}_output.png")
            gt_save_path = os.path.join(args.output_dir, f"{file_name[idx+i]}_gt.png")
            visualization_path = os.path.join(args.output_dir, f"{file_name[idx+i]}_visualization.png")
