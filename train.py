...
from perceptual import LossNetwork, edgeloss


import numpy as np
import random

from mymodel import AGEstormer
...
parser.add_argument('-theta_loss', help='Set the theta in loss function', default=0.05, type=float)
...
parser.add_argument('-debug', help='whether use debug mode', action="store_true")
parser.add_argument('-load_weight', help='whether to continue training', action="store_true")
...
root_dir = '/home/gagagk16/Rain/Derain/Dataset/RainGAN/'
...
# --- Define the network --- #
net = AGEstormer()
test_input_size=(1, 3, crop_size[0], crop_size[1])
summary_info = summary(net, input_size=test_input_size)
with open("./exp/{}/settings.txt".format(exp_name), "w") as f:
    f.write("Settings:\n")
...
vgg_model = vgg19(weights=VGG19_Weights.IMAGENET1K_V1).features[:16]
...
train_types = ["rainstreak"]
val_types = ["rainstreak"]
...
if DEBUG:
    train_dataset = create_random_subset(train_dataset, 0.5)
    val_dataset = create_random_subset(val_dataset, 0.5)
...
try:
    for epoch in range(epoch_start, num_epochs):
        ...
        current_lr = adjust_learning_rate(optimizer, epoch, learning_rate, eta_min= 0.2*learning_rate, T_max=200, warmup_epochs=5, use_cos=True)
    ...
    #-------------------------------------------------------------------------------------------------------------
        for batch_id, train_data in enumerate(train_dataloader):
            ...
            # edge_loss = edgeloss(pred_image, gt)
