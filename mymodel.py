...
class PatchEmbed(nn.Module):
    def __init__(self, img_size=448, patch_size=7, stride=4, in_chans=3, embed_dim=768):
        super().__init__()
...
class DetailEnhancedBlock(nn.Module):
    def __init__(self, dim, num_heads=8, mlp_ratio=4., qkv_bias=True, qk_scale=None,
        ...
        # 细节注意力模块
        self.detail_conv = nn.Conv2d(dim, dim, kernel_size=detail_kernel_size, padding=detail_kernel_size // 2, groups=dim) # TODO: refine this
...
class AGEstormer(nn.Module):
    def __init__(self, in_chans=3, img_size=448, embed_dims=[64, 128, 256, 512], num_heads=[1, 2, 4, 8],
                 mlp_ratio=[2,2,2,2], blocks_num=[2, 3, 3, 3], drop_path_rate=0.1, refine_blocks=3):
        super().__init__()
        ...
        self.embed_layer0 = PatchEmbed(img_size=img_size, patch_size=3, stride=2, in_chans=in_chans, embed_dim=embed_dims[0])

        self.embed_layer1 = PatchEmbed(img_size=img_size, patch_size=3, stride=2, in_chans=embed_dims[0], embed_dim=embed_dims[1])
        self.embed_layer2 = PatchEmbed(img_size=img_size, patch_size=3, stride=2, in_chans=embed_dims[1], embed_dim=embed_dims[2])
        self.embed_layer3 = PatchEmbed(img_size=img_size, patch_size=3, stride=2, in_chans=embed_dims[2], embed_dim=embed_dims[3])

        self.encoder1 = self._make_blk_enc(blocks_num[0], embed_dims[0], num_heads[0], mlp_ratio[0], drop_path=dpr1, detail_kernel_size=7)
        self.encoder2 = self._make_blk_enc(blocks_num[1], embed_dims[1], num_heads[1], mlp_ratio[1], drop_path=dpr2, detail_kernel_size=5)
        self.encoder3 = self._make_blk_enc(blocks_num[2], embed_dims[2], num_heads[2], mlp_ratio[2], drop_path=dpr3, detail_kernel_size=5)

        self.bottleneck = self._make_blk_enc(blocks_num[3], embed_dims[3], num_heads[3], mlp_ratio[3], drop_path=dpr4, detail_kernel_size=3)

        self.decoder3 = self._make_blk_dec(blocks_num[2], embed_dims[2], num_heads[2], mlp_ratio[2], detail_kernel_size=5)
        self.decoder2 = self._make_blk_dec(blocks_num[1], embed_dims[1], num_heads[1], mlp_ratio[1], detail_kernel_size=5)
        self.decoder1 = self._make_blk_dec(blocks_num[0], embed_dims[0], num_heads[0], mlp_ratio[0], detail_kernel_size=7)
        ...
        self.refinement = self._make_refinement(refine_blocks, embed_dims[0], num_heads=8, mlp_ratio=mlp_ratio[0])
